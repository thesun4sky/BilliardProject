...
bool MakeAverageSigma(ushort* volume, int dim[3], float* Average, float* Sigma, int cubeSize)
{
	...
	dim3 Dbx = dim3(16, 16);		// block dimensions are fixed to be 512 threads
    dim3 Dgx = dim3((fy+Dbx.x-1)/Dbx.x, (fz+Dbx.y-1)/Dbx.y);

	dim3 Dby = dim3(16, 16);		// block dimensions are fixed to be 512 threads
    dim3 Dgy = dim3((fz+Dby.x-1)/Dby.x, (fx+Dby.y-1)/Dby.y);

	dim3 Dbz = dim3(16, 16);		// block dimensions are fixed to be 512 threads
